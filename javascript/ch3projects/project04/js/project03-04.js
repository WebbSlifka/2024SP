...
let reviewTitles = ["My Favorite Workout Game", "Poor Choreography", "Buggy with Poor Tech Support", "Nice Improvement"];
  ...
  for (let i = 0; i < stars.length; i++) {
      console.log(starImages(stars[i]));
  }

  function starImages(rating) {
      let imageText = ""; 
      for (let i = 1; i <= rating; i++) {
          imageText += "<img src='/img/star.png' alt=''>"; 
      }
      return imageText; 
  }
...
for (let i = 0; i < reviewers.length; i++) {
    ...
    tableHTML += "<td>" + reviewers[i] + "</td>"; 
    tableHTML += "<td>" + reviewType[i] + "</td>"; 
    tableHTML += "<td>" + stars[i] + "</td>"; 
    tableHTML += "<td>" + reviewDates[i] + "</td>"; 
    tableHTML += "<td>" + reviewTitles[i] + "</td>"; 
    tableHTML += "<td>" + reviews[i] + "</td>"; 
    
    tableHTML += "<td>" + starImages(stars[i]) + "</td>";
...
for (let i = 0; i < reviewers.length; i++) {
      ...
      if (articleTags.length > 0) {
          articleTags[0].insertAdjacentHTML("beforeend", reviewCode);
      }
  }
