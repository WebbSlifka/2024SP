...
// Array of phone models sold by the company (fix: use square brackets for array initialization)
...
sales.forEach(addToTotal); // fix: function name should match the defined function

// For loop to generate bar chart of phone sales
for (let i = 0; i < phones.length; i++) { // fix: change condition to < instead of <=
   ...
   let barPercent = (100 * sales[i]) / totalSales; // fix: divide sales[i] by totalSales
   ...
   switch (phones[i]) { // fix: change i to phones[i]
   ...
   for (let j = 0; j < barPercent; j++) { // fix: change condition to < instead of <=
      barChart += cellTag + "</td>";
   }
   barChart += "</tr>";

   // Insert the bChart HTML code into first tbody element in the web document
   document.getElementsByTagName("tbody")[0].insertAdjacentHTML("beforeend", barChart); // fix: use square brackets for index and correct method name
}

// Callback function that adds the value of each array element to the totalSales variable
function addToTotal(arrValue) { // fix: function name corrected
   totalSales += arrValue;
}
